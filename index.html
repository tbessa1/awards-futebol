...
<body class="h-full overflow-auto gradient-bg">
    ...
    <script type="module">
        ...
        // Nota: Se estiver a carregar para o GitHub, estas variáveis devem ser substituídas 
        // pelos valores reais do seu projeto Firebase Console.
        const firebaseConfig = typeof __firebase_config !== 'undefined' ? JSON.parse(__firebase_config) : {
            apiKey: "A_SUA_API_KEY",
            authDomain: "O_SEU_PROJETO.firebaseapp.com",
            projectId: "O_SEU_PROJETO",
            storageBucket: "O_SEU_PROJETO.appspot.com",
        ...
        const appId = typeof __app_id !== 'undefined' ? __app_id : 'pinto-basto-awards-prod';
        ...
        async function init() {
            try {
                ...
                console.error("Erro Auth - Verifique se o Auth Anónimo está ativo no Firebase", e); 
            }
        ...
        function setupListeners() {
            ...
            }, (err) => console.error("Erro na BD Players:", err));
        ...
        window.loginAsAdmin = () => {
            if(document.getElementById('admin-password').value === ADMIN_PASS) {
                ...
                refreshUI();
            } else { alert("Palavra-passe de administração incorreta"); }
        };
        ...
        function renderAdmin() {
            ...
            pList.innerHTML = players.map(p => `
                <div class="flex justify-between items-center bg-white/5 p-3 rounded-xl border border-white/5">
                    ...
                    <button onclick="window.deletePlayer('${p.id}')" class="text-red-500 text-[10px] font-black p-2">X</button>
                </div>
            `).join('');
            ...
            gList.innerHTML = games.map(g => `
                <div class="flex justify-between items-center bg-white/5 p-3 rounded-xl border border-white/5">
                    ...
                    <button onclick="window.deleteGame('${g.id}')" class="text-red-500 text-[10px] font-black p-2">X</button>
                </div>
            `).join('');
        }
        ...
        function renderPlayer() {
            ...
            if(myGames.length === 0) {
                list.innerHTML = '<p class="text-center text-slate-500 text-[10px] uppercase py-10">Não estás convocado para nenhum jogo recente.</p>';
                return;
            }
            ...
            list.innerHTML = myGames.map(g => {
                ...
                return `
                    <div class="bg-[#0f172a] p-6 rounded-[2rem] border border-white/5 flex justify-between items-center">
                        ...
                        ${concluido ? 
                            '<span class="text-green-500 text-[9px] font-black uppercase bg-green-500/10 px-3 py-1 rounded-full border border-green-500/20">✓ Votado</span>' : 
                            `<button onclick="window.openVoting('${g.id}')" class="bg-blue-600 px-6 py-3 rounded-xl text-white font-black text-[9px] uppercase italic transition-transform active:scale-95">Votar</button>`
                        }
                    </div>
                `;
            }).join('');
        ...
        function renderRanking() {
            ...
            if (tableBody) {
                tableBody.innerHTML = stats.map((p, i) => `
                    <tr class="hover:bg-white/5 transition-colors">
                        <td class="px-6 py-4"><span class="w-6 h-6 rounded bg-white/5 flex items-center justify-center text-[10px] font-bold">${i+1}</span></td>
            ...
            if(pod && stats.length > 0) {
                const top = stats.slice(0, 3);
                pod.innerHTML = top.map((p, i) => {
                    const orderClass = i === 0 ? 'order-2 scale-110 mb-6' : (i === 1 ? 'order-1' : 'order-3');
                    const medal = i === 0 ? '🥇' : (i === 1 ? '🥈' : '🥉');
                    const borderColor = i === 0 ? 'border-yellow-500 shadow-yellow-500/20 shadow-xl' : 'border-white/10';
                    return `
                        <div class="flex flex-col items-center gap-2 ${orderClass}">
                            <div class="w-20 h-20 rounded-3xl bg-[#0f172a] border-2 ${borderColor} flex items-center justify-center text-2xl">
                                ${medal}
                            </div>
                            <p class="text-[10px] font-black uppercase text-center">${p.name.split(' ')[0]}</p>
        ...
        window.savePlayer = async () => {
            ...
            } catch (err) { console.error("Erro ao guardar jogador:", err); }
        };

        window.deletePlayer = async (id) => {
            if(confirm("Tem a certeza que deseja remover este atleta?")) {
        ...
        window.saveGame = async () => {
            ...
            } catch (err) { console.error("Erro ao criar jogo:", err); }
        };

        window.deleteGame = async (id) => {
            if(confirm("Deseja apagar este histórico de jogo?")) {
                                    ...
                                    <div class="flex gap-1 overflow-x-auto pb-2">
                                        ${[0,1,2,3,4,5].map(n => `
                                            <button onclick="window.submitVote('${gameId}', '${p.id}', '${cat.id}', ${n}, this)" 
                                                class="rating-btn w-8 h-8 shrink-0 rounded-lg text-[10px] font-bold border border-white/10 transition-all ${score === n ? 'active' : 'bg-black/20'}">
                                                ${n}
                                            </button>
                                        `).join('')}
                                    </div>
        ...
        window.openGameModal = () => {
            document.getElementById('call-up-selection').innerHTML = players.map(p => `
                <label class="flex items-center gap-2 p-2 text-[10px] font-bold uppercase cursor-pointer hover:bg-white/5 rounded-lg">
